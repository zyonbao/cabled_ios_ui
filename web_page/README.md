...
`web_page` 是基于 `ios_toolkit` 能力层的浏览器端交互控制台：在网页里实时镜像 USB 连接的 iOS 设备画面，并用鼠标进行点按 / 滑动操作。

本目录与 `ios_toolkit` 同级，运行时通过 `ios_toolkit.toolkit_api` 复用其设备管理、端口转发与 WDA 生命周期逻辑。
...
先安装 `ios_toolkit` 的依赖，再安装本目录的 Web 依赖：

```bash
python3 -m pip install -r ios_toolkit/requirements.txt
python3 -m pip install -r web_page/requirements.txt
```

## 启动

在仓库根目录（`ios_toolkit` 与 `web_page` 的父目录）执行：

```bash
python3 -m web_page.web_server            # 默认 http://127.0.0.1:8787
python3 -m web_page.web_server --port 9000
```

iOS 17+ 设备仍需先运行 `ios_tunneld`（见 `ios_toolkit/README.md`）。
