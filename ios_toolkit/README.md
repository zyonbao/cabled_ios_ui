...
`ios_toolkit` 是 Studio 平台协议下的 iOS Python 执行器实现，提供可复用的 `toolkit_api.py` 能力层和一次性 JSON CLI 入口 `toolkit_cli.py`。
...
ios_toolkit/
  ...
  tunneld_main.py    # iOS 17+ XPC tunnel daemon 入口，可打包为独立 tunneld 二进制
  ios_tunneld.py     # 仅非打包(dev)模式的 tunneld 启动器，须以 -m 模块形式运行
...
- Python 依赖：见 `ios_toolkit/requirements.txt`
...
```bash
python3 -B -m ios_toolkit.toolkit_cli
```
...
```bash
echo '{"op":"tap","args":{"target":"<UDID>","x":100,"y":200}}' | python3 -B -m ios_toolkit.toolkit_cli
```
...
echo '{"op":"type_credential","args":{"target":"<UDID>","env":"dev","role":"host","field":"password"}}' | python3 -B -m ios_toolkit.toolkit_cli
```
