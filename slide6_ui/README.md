...
`slide6_ui` 是基于 PySide6 的 iOS 设备桌面控制台，是 `web_page` 的桌面对等实现：在窗口里实时镜像 USB 连接的 iOS 设备画面，并用鼠标点按 / 滑动、宿主键盘输入进行操作。界面采用多 Tab 布局——「键鼠操作」承载镜像与键鼠控制，「App 列表」承载 App 管理与沙盒文件管理，「文件系统」承载设备媒体分区（不含 App 沙盒）的 AFC 文件管理，「相册」承载 DCIM 媒体的缩略图浏览 / 查看 / 导出 / 删除。

与 `web_page` 不同，它**在进程内直接复用 `ios_toolkit.toolkit_api`**（无 HTTP 层），并能在选中 iOS 17+ 设备时检测并经系统授权拉起 XPC tunnel。
...
slide6_ui/
  ...
  main_window.py      # 主窗口：顶栏 + Tab 容器 + 设备列表/选择委托
  common/             # 跨 Tab 公用组件
    afc_browser.py    # 可复用 AFC 浏览面板 AfcBrowserPanel + 对话框包装 AfcBrowserDialog
    sidebar_tabs.py   # 左侧竖排 Tab 容器 SidebarTabs
    tunnel.py         # XPC tunnel 检测与授权拉起/停止
    workers.py        # 线程池封装阻塞的 ios_toolkit.toolkit_api 调用
  device_info/        # 「设备信息」Tab（DeviceInfoTab）
    device_info.py
  album/              # 「相册」Tab（DcimAlbumTab）：DCIM 缩略图网格 + 本地缓存 + 大图查看 + 导出
    dcim_album.py
  file_system/        # 「文件系统」Tab（FileSystemTab）：root="media" 的 AFC 浏览（媒体分区，不含沙盒）
    file_system_tab.py
  app_manager/        # 「App 列表」Tab（AppManagerTab）：App 装卸/搜索/筛选 + 文件浏览器对话框
    app_manager.py
  keymouse/           # 「键鼠操作」Tab（KeymouseTab）
    keymouse_tab.py   # 镜像 + WDA/tunnel 生命周期 + 手势/键盘/动作/剪贴板
...
- macOS
- 先安装 `ios_toolkit` 依赖，再安装本目录的 PySide6 依赖：

```bash
python3 -m pip install -r ios_toolkit/requirements.txt
python3 -m pip install -r slide6_ui/requirements.txt
```

## 启动

在仓库根目录（`ios_toolkit` 与 `slide6_ui` 的父目录）执行：
...
- 采用 `osascript ... with administrator privileges` 按需拉起 `ios_toolkit.tunneld_main`，**每次拉起都会弹一次系统授权框**（不做持久化）。
...
- 仅支持 macOS（tunnel 授权为 macOS 专有；`ios_toolkit` 当前也仅支持 macOS USB 真机）。
- 不处理横屏 / 朝向（与 `web_page` 现状一致）。
