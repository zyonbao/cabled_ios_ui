...
- 增加事件流采集句柄：基于 `NetworkMonitor`（`startMonitoring`/`stopMonitoring`，事件推送、**无设备侧采样间隔**），Start/Stop/Pause/Clear 控制、窗口关闭自动回收、与窗口生命周期强绑定。
- 把接口/连接/连接更新事件归一化为：连接流（以 `connection_serial` 关联 detection 与 update）、上/下行速率（由连接 update 字节增量聚合）与「远端 IP/接口」聚合 TopN。
- 真机已确认：协议 `kind` **1=TCP、2=UDP**；**进程归属不可用（`pid` 恒为 -2）**，故去掉进程列表/进程过滤。
- 实现三栏主布局（进程归属不可用，见下，左栏改为按远端/接口聚合）：
  - 左栏：按「远端 IP/接口」聚合的 TopN（按字节/连接数）与 `IP:port` 搜索；
  - 中栏：连接流列表（时间/协议/方向/本地-远端 `IP:port`/字节）；
  - 右栏：详情与趋势图（Rx/Tx 速率、连接数、错误数=重传/重复）。
- 实现控制栏高频操作：Start/Stop、Pause、Clear、Auto-scroll、Export（CSV/JSON，先保留 PCAP 关联扩展位）。
- 实现过滤器：协议（TCP/UDP/unknown）、host/port（`IP:port` 子串）、时间窗口、关键词、仅活跃连接；方向（in/out）为推导值，过滤可选。进程过滤不提供（进程归属不可用）。
- 落地 10 分钟滚动窗口 + ring buffer + 后台事件队列上限（溢出丢最旧），超过窗口数据即时淘汰。
- 统一降级语义：协议/方向/进程名等字段缺失显示 `unknown/unsupported`，不中断整个会话。
...
- 代码：
  ...
  - `ios_toolkit/toolkit_api.py`（新增 `open_network_stream(target)` 入口，返回事件流句柄）
  - `ios_toolkit/device.py`（`NetworkMonitor` 事件流句柄：事件归一化、按 serial 聚合、远端/接口聚合、队列上限与回收）
