...
当用户确认启动 tunnel 时，应用 SHALL 通过 macOS 系统原生授权（osascript，管理员权限）拉起 `executor_ios.tunneld_main`，且被执行的命令路径固定、不拼接任何外部输入。该方案为按需拉起，每次启动 tunnel 均触发一次系统授权。

#### Scenario: 用户确认并授权成功

- **WHEN** 用户在提示中点击确认并通过系统授权框完成授权
- **THEN** 以管理员权限启动 `tunneld_main`
...
- **THEN** 应用停止该 tunneld 进程（优先用本会话记录的 pid，否则解析占用端口的监听 pid）
...
拉起 tunneld 的过程 SHALL 使用系统原生授权框，校验被执行脚本/二进制路径的存在性，命令字符串不得包含任何用户输入，且不向该进程传递任何凭据。

#### Scenario: 命令路径固定且校验

- **WHEN** 应用构造用于授权拉起的命令
- **THEN** 命令仅由内部固定的解释器/二进制路径与 `tunneld_main` 入口组成
