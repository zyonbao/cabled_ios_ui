...
桌面应用 SHALL 在 Settings 窗口的「General」标签中提供日志配置（位于配置文件路径 section 之下），包含：启用文件日志的开关，以及日志目录的选择（可经文件选择器浏览，亦可直接填写路径）。这些设置 MUST 经 `QSettings` 持久化（键 `settings/logging_enabled`、`settings/logging_dir`），下次启动 SHALL 沿用。日志目录为空时 MUST 回退到默认目录 `~/Library/CablediOS/Logs`，且目录输入框 MUST 以占位文案直接展示该默认路径（形如「默认:~/Library/CablediOS/Logs」）。

#### Scenario: 配置并持久化

- **WHEN** 用户在 General 标签的日志区勾选启用并选择/填写日志目录
...
#### Scenario: 日志配置不再位于独立 Logging 标签

- **WHEN** 用户打开 Settings 查找日志配置
- **THEN** 日志的启用开关与目录选择位于 `General` 标签内，且不存在独立的 `Logging` 标签
