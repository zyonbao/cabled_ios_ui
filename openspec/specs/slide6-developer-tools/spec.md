...
「条件诱导」功能位 SHALL 提供条件组/profile 选择入口（预定义 profile，无自定义参数），用于施加设备诱导条件（如弱网 / 热状态 / GPU 性能档，按设备返回动态呈现并过滤 internal 项）。界面 MUST 显示当前诱导状态（未启用 / 已启用 + 条件名称/摘要）；用户 MUST 可开始诱导与结束诱导。设备同一时刻仅允许一个活动条件，切换 profile MUST 先结束旧条件再施加新条件。开始诱导前 SHOULD 显示条件确认（`isDestructive` 项显著标注）；结束诱导后 MUST 刷新状态并清理句柄资源。条件为连接作用域，子面板关闭/连接断开时设备 MUST 自动恢复（详见 `dvt-condition-inducer-op`）。

#### Scenario: 开始诱导

- **WHEN** 用户选择 profile 并点击开始
- **THEN** 应用施加该条件并显示「已启用」状态及条件名称/摘要
