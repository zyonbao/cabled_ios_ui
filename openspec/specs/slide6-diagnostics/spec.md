...
「诊断」Tab 的全部功能在 iOS 17+ 设备上经 RSD 访问，依赖 XPC tunnel。该 Tab SHALL 在顶部提供 XPC tunnel 状态条，**仅当目标设备为 iOS 17+ 时显示**：展示 tunnel 运行状态，未运行时提供「启动」入口，运行时提供「停止」「重启」入口（复用共享的 tunnel 助手）。当目标设备需要 tunnel 且 tunnel 未运行时，全部功能卡片 MUST 置为 Disabled 并以 tooltip 说明缺失项；tunnel 启动后 MUST 自动启用卡片。iOS < 17 设备 MUST NOT 显示该状态条，且卡片不因 tunnel 状态被门控。

#### Scenario: iOS 17+ 缺 tunnel 时禁用

- **WHEN** 目标设备 iOS ≥ 17 且 XPC tunnel 未运行
- **THEN** 顶部显示 tunnel 状态条（含启动入口），全部功能卡片置为 Disabled 并以 tooltip 说明需先启动 tunnel

#### Scenario: 启动 tunnel 后启用

- **WHEN** 用户在状态条启动 XPC tunnel 并成功
- **THEN** 状态条刷新为「运行中」，全部功能卡片自动启用

#### Scenario: iOS 17 以下不门控

- **WHEN** 目标设备 iOS < 17
- **THEN** 不显示 tunnel 状态条，功能卡片不因 tunnel 状态被禁用
