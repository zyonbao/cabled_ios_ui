...
## Purpose
定义桌面端 `slide6_ui` 的国际化（i18n）能力：以 `slide6_ui/languages/` 下的 JSON catalog 为唯一文案来源，提供启动期一次性选定语言的国际化工具 `i18n.py`（取值、回退、具名占位符插值与一致性校验），并要求展示层所有面向用户文案统一经由 `t(...)` 取值，不保留硬编码展示字符串。默认 `zh-CN`，受支持语言为 `zh-CN` / `en-US`，语言切换为重启生效。
