# afc-filesystem-op Specification

## Purpose
定义 executor 层经 `com.apple.afc`（lockdown 直连）访问设备媒体分区根（不含 App 沙盒）的能力：`root="media"` 下的目录浏览、文件/文件夹导入(push)/导出(pull)、删除、新建目录、重命名，以及按需读取文件字节的 `afc_read`（供缩略图加载与回退解码，受 `max_bytes` 上限保护）。
## Requirements
